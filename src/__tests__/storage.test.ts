...
describe('Storage Adapters', () => {
  describe('AsyncStorageAdapter', () => {
    ...
    it('should get item with prefix', async () => {
      ...
      expect(AsyncStorage.getItem).toHaveBeenCalledWith('@omam-authkit:test-key');
      expect(value).toBe('test-value');
    });

    it('should set item with prefix', async () => {
      await adapter.setItem('test-key', 'test-value');
      expect(AsyncStorage.setItem).toHaveBeenCalledWith('@omam-authkit:test-key', 'test-value');
    });

    it('should remove item with prefix', async () => {
      await adapter.removeItem('test-key');
      expect(AsyncStorage.removeItem).toHaveBeenCalledWith('@omam-authkit:test-key');
    });
  ...
  describe('SecureStoreAdapter', () => {
    ...
    it('should get item with prefix', async () => {
      ...
      const value = await adapter.getItem('test-key');
      expect(SecureStore.getItemAsync).toHaveBeenCalledWith('omam-authkit:test-key');
      expect(value).toBe('test-value');
    });

    it('should set item with prefix', async () => {
      await adapter.setItem('test-key', 'test-value');
      expect(SecureStore.setItemAsync).toHaveBeenCalledWith('omam-authkit:test-key', 'test-value');
    });

    it('should remove item with prefix', async () => {
      await adapter.removeItem('test-key');
      expect(SecureStore.deleteItemAsync).toHaveBeenCalledWith('omam-authkit:test-key');
    });
